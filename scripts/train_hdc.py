...
def _resolve_feature_names_hdc(cfg: dict) -> list[str]:
...
def _extract_test_arrays_hdc(task: dict) -> tuple[np.ndarray, np.ndarray]:
...
def main() -> None:
    ...
    try:
        ...
        feature_names = _resolve_feature_names_hdc(cfg)
        # En mode classification (non one_class_mode), utiliser predict (retourne 0/1).
        # En mode one_class, utiliser anomaly_score avec le seuil appris.
        one_class = getattr(model, "one_class_mode", False)
        if one_class and model.anomaly_threshold_ is not None:
            predict_fn_fi = model.anomaly_score
            threshold_fi = float(model.anomaly_threshold_)
        else:
            predict_fn_fi = lambda X: model.predict(X).astype(float)
            threshold_fi = 0.5

        task_arrays_fi: list[dict] = []
        for t in tasks:
            X_t, y_t = _extract_test_arrays_hdc(t)
        ...
        global_perm = permutation_importance(
            predict_fn_fi, X_all_fi, y_all_fi, feature_names, threshold=threshold_fi
        )
        per_task_perm = permutation_importance_per_task(
            predict_fn_fi, task_arrays_fi, feature_names, threshold=threshold_fi
        )
        fmi = feature_masking_importance(
            predict_fn_fi, X_all_fi, y_all_fi, feature_names, threshold=threshold_fi
        )
        ...
        importance_results = {
            ...
            "global": {
                ...
                "feature_masking_importance": fmi,
            },
            "per_task": {
