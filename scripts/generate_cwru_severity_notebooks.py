"""
generate_cwru_severity_notebooks.py

Génère les 7 notebooks d'analyse CL pour le scénario cwru_by_severity.
    notebooks/cl_eval/cwru_by_severity/{ewc,hdc,tinyol,kmeans,mahalanobis,dbscan,comparison}.ipynb

Usage :
    python scripts/generate_cwru_severity_notebooks.py
"""
...
OUT_DIR = REPO_ROOT / "notebooks/cl_eval/cwru_by_severity"
...
MODEL_CONFIGS = [
    {
        "model_name": "EWC",
        "model_key": "ewc",
        "exp_id": "exp_080_ewc_cwru_by_severity",
        "exp_num": "080",
        "fault_type_exp": "exp_074_ewc_cwru_by_fault_type",
        "fault_type_num": "074",
        ...
        "sprint_ref": "12 — S12-08",
        "ragged_acc_matrix": False,
    },
    {
        "model_name": "HDC",
        "model_key": "hdc",
        "exp_id": "exp_081_hdc_cwru_by_severity",
        "exp_num": "081",
        "fault_type_exp": "exp_075_hdc_cwru_by_fault_type",
        "fault_type_num": "075",
    ...
    },
    {
        "model_name": "TinyOL",
        "model_key": "tinyol",
        "exp_id": "exp_082_tinyol_cwru_by_severity",
        "exp_num": "082",
        "fault_type_exp": "exp_076_tinyol_cwru_by_fault_type",
        "fault_type_num": "076",
    ...
    },
    {
        "model_name": "KMeans",
        "model_key": "kmeans",
        "exp_id": "exp_083_kmeans_cwru_by_severity",
        "exp_num": "083",
        "fault_type_exp": "exp_077_kmeans_cwru_by_fault_type",
        "fault_type_num": "077",
    ...
    },
    {
        ...
        "exp_id": "exp_084_mahalanobis_cwru_by_severity",
        "exp_num": "084",
        "fault_type_exp": "exp_078_mahalanobis_cwru_by_fault_type",
        "fault_type_num": "078",
    ...
    },
    {
        "model_name": "DBSCAN",
        "model_key": "dbscan",
        "exp_id": "exp_085_dbscan_cwru_by_severity",
        "exp_num": "085",
        "fault_type_exp": "exp_079_dbscan_cwru_by_fault_type",
        "fault_type_num": "079",
...
def build_model_notebook(cfg: dict) -> dict:
    ...
    ft_exp = cfg["fault_type_exp"]
    ft_num = cfg["fault_type_num"]
    cl_strat = cfg["cl_strategy"]
    sprint = cfg["sprint_ref"]
    ragged = cfg["ragged_acc_matrix"]

    # --- Cell 0: header markdown ---
    header_md = f"""# Évaluation CL — {mn} — Dataset CWRU Bearing — by_severity
...
jupyter nbconvert --to notebook --execute \\
    notebooks/cl_eval/cwru_by_severity/{mk}.ipynb \\
    --output /tmp/{mk}_cwru_severity_executed.ipynb --ExecutePreprocessor.timeout=300
```"""
...
FIGURES_DIR = REPO_ROOT / "notebooks/figures/cl_evaluation/{mk}/cwru/by_severity"
FIGURES_DIR.mkdir(parents=True, exist_ok=True)

BASELINE_DIR   = REPO_ROOT / "experiments/{bline_exp}/results"
FAULT_TYPE_DIR = REPO_ROOT / "experiments/{ft_exp}/results"
...
display(Image(str(FIGURES_DIR / "accuracy_matrix.png")))"""
    ...
    # --- Cell 4: per-task barplot (severity gradient colors) ---
    barplot_code = f"""\
...
display(Image(str(FIGURES_DIR / "ram_vs_budget.png")))"""
    ...
    # --- Cell 7: summary table (3 columns: by_severity | by_fault_type | baseline ST) ---
    summary_code = f"""\
...
if fault_type_path.exists():
    ...
    display(Markdown("> ⚠️ by_fault_type ({ft_exp}) non disponible."))
    aa_ft = float("nan"); af_ft = float("nan"); bwt_ft = float("nan")
    ram_ft = ram_b; lat_ft = lat; n_par_ft = n_par

display(Markdown(f"### Résultats finaux — {{MODEL_NAME}} — cwru/by_severity ({eid})"))
...
ax_t.axis("off")
col_labels = ["Métrique", "CL by_severity ({mn})", "CL by_fault_type", "Baseline single-task"]
row_data = [
