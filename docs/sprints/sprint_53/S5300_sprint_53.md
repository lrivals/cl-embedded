...
| B3 ✅ | **Isolation à variable unique de l'effet « build S38 »** — la comparaison `EWC_IN=4` vs `5` s'est révélée **non réalisable** (garde `EWC_IN == EWC_HEAD_NATIVE_DIM`, et une couche `k×32` n'a pas le même contenu à deux dimensions) ; substituée par **dimension tenue à k=4, poids exportés contre repli Xavier** | 2 flashes, fait | **Poids écartés comme cause** : les deux variantes sont plates (−1,07 ± 0,41 et +1,67 ± 0,75 µA/Hz, r² 0,635 et 0,554) contre +20,73 sur le build stock. `.bss` identique au bit près, 3 072 B de `.text` d'écart (les seules tables de poids). Δ à 3,2 σ **non publié** : la règle A4 exige la linéarité de chaque régression. → `experiments/exp_S53_build_isolation/`, détail dans S5306. |
...
| B8 | **Paire (ii)** — Xavier des deux côtés, `EWC_IN=4` contre `EWC_IN=6` | 2 flashes, ~30 min | Née de B3 : confirmerait, **dans une séance unique**, que l'effet suit la dimension. Un indice inter-séances existe déjà (le build « dimensions par défaut », Xavier lui aussi, donnait +17,96 µA/Hz à r² 0,979) mais il n'a pas été mesuré à conditions appariées. |
