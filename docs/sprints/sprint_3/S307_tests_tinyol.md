...
Créer `tests/test_tinyol.py` couvrant les composants TinyOL : autoencoder backbone, tête OtO, et boucle online. La suite doit être autonome (fixtures sans données réelles) et passer sans le Dataset 1 téléchargé.

**Critère de succès** : `pytest tests/test_tinyol.py -v` retourne ≥ 10 tests passants en < 30 secondes sur CPU.

---

## Sous-tâches

### 1. Structure du fichier de tests

```python
# tests/test_tinyol.py
"""
...
def default_config() -> dict:
    """Configuration minimale pour les tests (identique à tinyol_config.yaml)."""
    return {
        "backbone": {
            "input_dim": 25,
...
@pytest.fixture
def oto_head(default_config) -> OtOHead:
    return OtOHead(input_dim=default_config["online"]["oto_input_dim"])
...
### 6. Test budget mémoire (intégration légère)
...
```bash
# Prérequis : S3-03 (autoencoder.py) + S3-05 (oto_head.py) terminés
pip install -e ".[dev]"

# Lancer tous les tests TinyOL
pytest tests/test_tinyol.py -v

# Lancer uniquement les tests OtO
pytest tests/test_tinyol.py -k oto -v

# Lancer avec couverture
pytest tests/test_tinyol.py --cov=src/models/tinyol --cov-report=term-missing
```
...
- `TODO(arnaud)` : faut-il ajouter un test d'intégration qui vérifie la cohérence `PumpMaintenanceDataset → TinyOLOnlineTrainer.update` sur quelques échantillons synthétiques (sans télécharger le dataset réel) ?
- `TODO(arnaud)` : le test `test_update_modifies_oto_weights` peut échouer si le gradient est nul (ex. saturation sigmoid). Faut-il initialiser les poids OtO avec une seed fixe dans la fixture pour garantir la reproductibilité du test ?
