...
| **Dépendances** | Sprint 16 (toolchain + UART MVP ✅), Sprint 17 (NUCLEO périphériques ✅) |
...
**Critère de succès** : `python scripts/board_dataset_builder.py --dry-run` produit un `experiments/exp_S18_01/results.json` et un `dataset.csv` valides en moins de 5 minutes, sans board. ✅ **Atteint** (1.1 s).

**Validation board** : `exp_S18_01_board/` produit le 2026-05-25 — lat=3.7 µs, RAM=1000 B, Gap 2 compliant. ✅
...
| S1807 | Tests Unity firmware : DWT latence, CRC frames, buffer overflow guards | 🟡 | ✅ | `firmware/stm32f4_blink/tests/test_profiling.c`, `test_pipeline.c` | S1804 |
...
- ~~`TODO(dorra)` : NUCLEO-F439ZI disponible pour tests série complète ?~~ ✅ **Board validée le 2026-05-25** — pipeline bout-en-bout opérationnel.
