...
- [ ] `from src.models.unsupervised import PCABaseline` — aucune erreur d'import
- [ ] `fit_task(X, task_id=0)` avec `n_components=2` — PCA entraîné, seuil calculé
- [ ] `fit_task(X, task_id=0)` avec `n_components=None` — sélection automatique retourne un entier ∈ [1, n_features]
- [ ] `reconstruction_error(X)` retourne `np.ndarray [N]` avec valeurs ≥ 0
- [ ] `predict(X)` retourne `np.ndarray [N]` avec valeurs ∈ {0, 1}
- [ ] `anomaly_score(X)` retourne dtype `float32`
- [ ] `score(X, y)` retourne float entre 0 et 1
- [ ] `cl_strategy="incremental"` — `pca_` est `IncrementalPCA`, `partial_fit` appelé
- [ ] `count_parameters()` = `n_components × n_features + n_features` (components_ + mean_)
- [ ] `ruff check src/models/unsupervised/pca_baseline.py` + `black --check` passent
- [ ] `save` + `load` round-trip : mêmes erreurs de reconstruction avant et après
