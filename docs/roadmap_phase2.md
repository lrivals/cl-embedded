...
Sprint 40 (article)      → Article standalone FR+EN « EWC INT8 sur MCU » (parité FP32 mesurée · effondrement PTQ naïve · récupération par kernel calibré · honnêteté mesuré board vs émulé PC)  ✅ (S4004–S4007 : `docs/article/ewc_int8_mcu/` classe `article`, `main_{fr,en}.tex` + 7 sections miroir + `references.bib` autonome + Makefile → **`make all` compile FR+EN sans erreur**, 5 figures S4003, chiffres FR≡EN adossés JSON S36/S39, board v2 « à mesurer » ; `test_sprint40_article.py` **14 PASS / 2 skips honnêtes** ; firmware inchangé) · **refonte S4008–S4010 ✅** : agrégat unique `exp_S40_article_metrics/summary.json` (238 cellules/jeu × 9 axes, 5 estimateurs d'énergie **jamais fusionnés**, `missing` = 28) + coût de calcul EWC (`measure_macs.py --head multiclass` → 672/704 MACs, BOPs ×16 théorique, `n_params` 722/754 **vérifiés contre S39**) ; catalogue `article_ewc_int8` **10 figures** + **synchronisation automatique** vers l'article (fin de la dérive md5 fig2/fig4/fig5) ; article étendu de 2 sections (`05b` moment+profondeur, `05c` budget système) et **récupération INT8 requalifiée « mesurée carte »** sur les 4 cellules per-canal (F1 0.9173/0.8995, parité gelée 1.000, 0 CRC) ; FR≡EN strict, PDF 15 p. 0 réf. indéfinie ; `test_sprint40_article.py` 15 PASS/2 skips banc + `test_article_metrics.py` 9 PASS + `test_figures_library.py` 15 PASS ; firmware inchangé `make test` 145 · **S4012 ✅ formalisation EWC** : sous-section 3.1 « Formulation EWC et adaptation embarquée » FR+EN — 4 équations (perte régularisée, Fisher diagonal empirique, pas SGD embarqué, consolidation `F ← α F + (1−α) W²`) + tableau `tab:ewc_port` PC/carte ; **le proxy de magnitude est nommé** (la carte n'estime pas le Fisher : ni donnée, ni étiquette, ni passe arrière), pénalité embarquée sans les biais, `F ≡ 0` avant première consolidation, λ 1000 PC vs 400 carte, rétropropagation en place rattachée à la parité en ligne ; clé `Schwarz2018Online` ajoutée (écart assumé à la liste S4004, 9→10 bibitems) ; PDF 27/26 p. 0 erreur LaTeX 0 réf. indéfinie, 24 PASS/2 skips banc + 17 PASS figures · **S4013 ✅ formalisation quantification** : sous-section « Formulation de la quantification » FR+EN — 6 équations (mapping symétrique `Q=clip(round(W/s))` `qmax=2^(b−1)−1`, **les 3 granularités comme UNE famille** `1/128` figée / `max|W|/qmax` / `max_i|W[j,i]|/qmax`, couche entière `acc_j·s_j·s_a+b_j` où **l'indice `j` EST le par-canal**, ternaire TWN seuil `0.7·mean|W|`, binaire BWN, activation affine) + tableau `tab:quant_ref` **schéma de référence `Jacob2018` vs noyau v2** (7 écarts assumés : déquant FPU vs multiplicateur entier — **origine du paradoxe de latence** ; biais FP32 ; pas de zéro-point poids ; calibration max absolu vs histogramme ; pas de fusion normalisation ; acc `int32`→`int64` Q15) + paragraphe coût du par-canal ; **coquille corrigée** (`legacy_c` décrit comme accumulateur `int8` → c'est un `int16` qui reboucle, 4 écarts cumulés énumérés) ; **biblio 9→15 clés** (`Jacob2018`, `Krishnamoorthi2018`, `Li2016TWN`, `Rastegari2016XNOR`, `Lai2018CMSISNN` + **journal `Ravaglia2021QLRCL` corrigé** TCSVT→JETCAS 11(4):789-802 +DOI) ; PDF 30/29 p. 0 erreur LaTeX 0 réf. indéfinie, 41 PASS/2 skips banc · **S4014 ✅ correctifs d'exactitude, sans nouvelle mesure** : (1) **frontière de profondeur requalifiée** — la conclusion « ternaire » reposait sur le seul ΔAUROC émulé alors que le bloc `depth_board` de l'agrégat porte le **F1 mesuré carte** des mêmes schémas, qui la contredit (Pronostia ternaire AUROC 0,99907 / **F1 0,3790**, binaire 0,96616 / **F1 0,0000** ; Monitoring binaire 0,96784 / 0,6878) ⇒ les deux lectures sont rapportées côte à côte, **frontière = INT4 en F1**, message reformulé en *couple profondeur–métrique de sélection* (motif « accuracy trompeuse → F1 » du Sprint 35) ; nouvelle table **générée depuis l'agrégat** `build_depth_board` → `tables/depth_board_{fr,en}.tex` (`tab:depth-board`), intro/conclusion/annexe/légende fig. 7 alignées FR≡EN ; (2) **portée de l'INT8 en ligne énoncée** — `ewc_head_int8_v2.h` n'expose qu'un forward et `pipeline.c` applique le pas de gradient à la **tête FP32 maîtresse** avant requantification ⇒ le ÷4 vaut pour l'inférence seule, la carte portant en apprentissage tête flottante + vue entière + importance + ancrage ; quantification *compatible* avec le CL, non *conduite* en entier (problème ouvert) ; (3) **latence d'apprentissage chiffrée** — 577/606 µs en ligne contre 239/251 FP32 = **facteur 2,4** (requantification intégrale par échantillon), jamais commenté alors qu'il figurait au tableau ; pistes ajoutées (requantification des seules lignes modifiées, SGD entier). Forme : `tab:parity` `lccccc`→`lcccc` (colonne fantôme), entête « F1 carte » + note Monitoring 4 features natives, intro 0,9626–0,9887 sur les 3 cellules, `05c` précise que la RAM des poids compte les matrices seules (672/704) hors biais (722/754 paramètres — **les deux chiffres étaient justes**). Tests : **43 PASS / 2 skips banc** (+`test_depth_reports_both_metrics`, +`test_online_int8_scope_is_stated`) ; `make all` **0 erreur LaTeX, 0 réf. indéfinie** (FR 32 p., EN 31 p.) ; **firmware non touché**
